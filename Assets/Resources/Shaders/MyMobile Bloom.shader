...
Shader "MyMobile/Bloom" {
	...
	CGINCLUDE
		...
		half4 fragBloom ( v2f_simple i ) : COLOR
		{	
			half4 color = tex2D(_MainTex, i.uv.xy);
			return color + tex2D(_Bloom, i.uv.zw);
		} 
		
		half4 fragBloomWithColorMix ( v2f_simple i ) : COLOR
		{	
			half4 color = tex2D(_MainTex, i.uv.xy);	
		...
		half4 fragMaxWithPain ( v2f_withMaxCoords i ) : COLOR
		{				
			half4 color = tex2D(_MainTex, i.uv.xy);
		...
		half4 fragMax ( v2f_withMaxCoords i ) : COLOR
		{				
			half4 color = tex2D(_MainTex, i.uv.xy);
			color = max(color, tex2D (_MainTex, i.uv2[0]));	
			color = max(color, tex2D (_MainTex, i.uv2[1]));	
		...
		half4 fragBlurForFlares ( v2f_withBlurCoords i ) : COLOR
		{				
			half4 color = tex2D (_MainTex, i.uv2[0]);
