...
public abstract class DCFG
{
    ...
    public static List<T>    ReadList<T>(XmlElement os) where T : DCFG
    {
        ...
        List<XmlElement> childs = GetChilds(os);
        for (int i = 0; i < childs.Count; i++)
        {
            XmlElement current = childs[i];
    ...
    public static void Write(XmlDocument doc, XmlElement os, string name, Vector4 v)
    {
        string s = string.Format("({0},{1},{2},{3})", v.x.ToString("0.00"), v.y.ToString("0.00"), v.z.ToString("0.00"), v.w.ToString("0.00"));
        os.SetAttribute(name, s);
    }
    ...
    public static void Write(XmlDocument doc, XmlElement os, string name, List<float> list)
    {
        string s = string.Empty;
        for (int i = 0; i < list.Count; i++)
        {
    ...
    public static void Write(XmlDocument doc, XmlElement os, string name, List<EItem> list)
    {
        string s = string.Empty;
        for (int i = 0; i < list.Count; i++)
        {
    ...
    public static void Write<T>(XmlDocument doc, XmlElement os, List<T> list) where T : DCFG
    {
        if (list.Count == 0)
        {
            return;
        ...
        {
            T obj = list[i];
            Write(doc, os, obj.GetType().Name, obj);
        }
    }
    ...
    public static List<XmlElement> GetChilds(XmlNode parent)
    {
        ...
        if (parent != null)
        {
            ...
            {
                XmlElement sub = parent.ChildNodes[i] as XmlElement;
                if (sub == null)
                {
                    continue;
                }
