...
public static class GTTools
{
    ...
    static Dictionary<Type, List<IExtensible>> m_ExtensionObjs = new Dictionary<Type, List<IExtensible>>();
    public enum Tags
    {
        MainCamera,
        UI,
    ...
    public static string Vector2ToString(Vector2 pos)
    {
        return Format("({0},{1})", pos.x.ToString("0.00"), pos.y.ToString("0.00"));
    }
    ...
    public static Vector3 GetClosestPointFromLineSegmentToPoint(Vector3 start, Vector3 end, Vector3 point)
    {
    ...
    public static float GetCurrentStateLength(this Animator animator, int layer)
    {
        if (animator != null)
        {
    ...
    public static T          GetExtensionObj<T>(int index = 0) where T: ProtoBuf.IExtensible
    {
        T t = default(T);
        Type type = typeof(T);
        List<IExtensible> objs = new List<IExtensible>();
        m_ExtensionObjs.TryGetValue(type, out objs);
        if (objs == null)
        {
            objs = new List<IExtensible>();
            m_ExtensionObjs.Add(type, objs);
        }
        if (objs.Count <= index)
        {
            for (int i = objs.Count; i < index + 1; i++)
            {
	...
	public static void InitListWithDefalut<T>(ref List<T> list, int nSize) 
	{
    ...
    public static void       AdaptListCount<T>(ref List<T> list, int count)
    {
        if (list.Count > count)
        {
