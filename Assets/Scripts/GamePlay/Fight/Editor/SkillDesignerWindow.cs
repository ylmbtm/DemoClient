...
{
    [MenuItem("编辑器/技能编辑器")]
    static void OpenSkillDesignerEditor()
    {
        SkillDesignerWindow window = GetWindow<SkillDesignerWindow>(true, "SkillDesignerWindow");
        window.Init();
        window.Show();
    }

    private Rect                         m_rectWindow;
    private Rect                         m_rectSkill;       //技能
    private Rect                         m_rectAction;      //事件属性
    private Rect                         m_recvActList;     //行为列表
    private Rect                         m_rectTime;
    private float                        m_heightTooBar = 0;
    ...
    {
        m_rectWindow = position;
        m_rectSkill = new Rect(0, m_heightTooBar, 300, position.height - m_heightTooBar);     //左侧
        m_recvActList = new Rect(0, m_heightTooBar, 300, position.height - m_heightTooBar);
        m_rectAction = new Rect(305, m_heightTooBar, 300, position.height - m_heightTooBar);  //中间
        m_rectTime = new Rect(620, m_heightTooBar, position.width - 630, position.height - m_heightTooBar); //右侧的时间
        m_TimeWidth = m_rectTime.width / m_TimeNum; //每一个时间间隔的宽度
        m_TimeMuti = m_TotalTime / m_rectTime.width; //每一个像素所表示的时间
        m_TimelineControl.Rect = new Rect(m_rectTime.x + m_TimelineControl.Time / m_TimeMuti, 0, 30, 30);
    ...
    {
        GUILayout.BeginArea(m_rectSkill, "", EBVTStyles.BVT_Window);
    ...
    {
        GUILayout.BeginArea(m_rectAction, "", EBVTStyles.BVT_Window);
        //BeginWindows();
    ...
    private void DrawTimeline()
    {
        ...
        {
            Vector2 p1 = new Vector2(m_rectTime.x + i * m_TimeWidth, 25);
            Vector2 p2 = new Vector2(m_rectTime.x + i * m_TimeWidth, position.height);
    ...
    private void DrawTimelineItems()
    {
        ...
        for (int i = 0; i < current.Items.Count; i++)
        {
            Rect trackRect = new Rect(m_rectTime.x, 30 + i * 50, m_rectTime.width, 45);
            ...
            float fh = 35;
            float fx = m_rectTime.x + item.StTime / m_TimeMuti;
            ...
            if (drawRect.x < m_rectTime.x)
            {
                drawRect.x = m_rectTime.x;
            }
            if (drawRect.x > m_rectTime.x + m_rectTime.width - drawRect.width)
            {
                drawRect.x = m_rectTime.x + m_rectTime.width - drawRect.width;
            }
    ...
    private void DrawTimelineControl()
    {
        ...
        {
            m_TimelineControl.Time = (e.mousePosition.x - m_rectTime.x) * m_TimeMuti;
            if (m_TimelineControl.Time < 0)
            {
                m_TimelineControl.Time = 0;
            }
            if (m_TimelineControl.Time > m_TotalTime)
            {
                m_TimelineControl.Time = m_TotalTime;
            }
            e.Use();
        }
