...
namespace MAP
{
    ...
    {
        public static void PathBezierInterp(float time, List<MAPPathNode> paths, out Vector3 p, out Quaternion r, out Vector3 s)
        {
        ...
        public static void PathLinearInterp(float time, List<MAPPathNode> paths, out Vector3 p, out Quaternion r, out Vector3 s)
        {
            if (time > paths[paths.Count - 1].Time)
            {
                p = paths[paths.Count - 1].transform.position;
