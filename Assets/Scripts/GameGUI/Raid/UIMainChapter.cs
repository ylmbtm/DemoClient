...
public class UIMainChapter : GTWindow
{
    ...
    private List<UIItemCopy> copyItems = new List<UIItemCopy>();
    ...
    protected override void OnAwake()
    {
        ...
        for (int i = 1; i <= 6; i++)
        {
            UIItemCopy item = new UIItemCopy();
    ...
    private void OnBoxClick(GameObject go)
    {
        ...
        ERewardState state = MLRaid.Instance.GetChapterRewardState(chapter, index);
    ...
    private void ShowViewByChapter(int chapter)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            ERewardState rewardState = MLRaid.Instance.GetChapterRewardState(chapter, i);
            switch(rewardState)
            {
        ...
        for (int i = 0; i < copyItems.Count; i++)
        {
            UIItemCopy tab = copyItems[i];
            if (i < chapterDB.Copys.Length)
            {
                tab.item.SetActive(true);
                Color color = Color.white;//i < enterIndex ? Color.white : Color.cyan;
                ...
                tab.copyName.color = Color.yellow;// i < enterIndex ? Color.yellow : Color.gray;
