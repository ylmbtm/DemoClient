...
public class UIItemUse : GTWindow
{
    ...
    private ulong     mCurrCount;
    private ulong     mMaxUseCount;
...
//         GTNetworkSend.Instance.TryUseItemByPos(mItemPos, mCurrCount);
    ...
    {
        GTAudioManager.Instance.PlayEffectAudio(GTAudioKey.SOUND_UI_CLICK);
        ulong showNum = mMaxUseCount - mCurrCount > 10 ? mCurrCount + 10 : mMaxUseCount;
    ...
    {
        GTAudioManager.Instance.PlayEffectAudio(GTAudioKey.SOUND_UI_CLICK);
        ulong showNum = mMaxUseCount - mCurrCount > 1 ? mCurrCount + 1 : mMaxUseCount;
        SetCurrCount(showNum);
    }
    ...
    {
        GTAudioManager.Instance.PlayEffectAudio(GTAudioKey.SOUND_UI_CLICK);
        ulong showNum = mCurrCount - 10 > 1 ? mCurrCount - 10 : 1;
        SetCurrCount(showNum);
    }
    ...
    {
        GTAudioManager.Instance.PlayEffectAudio(GTAudioKey.SOUND_UI_CLICK);
        ulong showNum = mCurrCount - 1 > 1 ? mCurrCount - 1 : 1;
        SetCurrCount(showNum);
    }
    ...
    private void SetCurrCount(ulong count)
    {
        mCurrCount = count;
        itemCurrCount.text = mCurrCount.ToString();
    }

    public void ShowView(ulong guid)
    {
        mGuid = guid;
        BagItem item = GTBagData.GetItem(guid);
        mMaxUseCount = GTBagData.GetItemCount(item.ItemID);

        GTItemHelper.ShowItemName(itemName, item.ItemID);
        GTItemHelper.ShowItemTexture(itemTexture, item.ItemID);
        GTItemHelper.ShowItemQuality(itemQuality, item.ItemID);
        SetCurrCount(1);
