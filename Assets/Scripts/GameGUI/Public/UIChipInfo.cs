...
public class UIChipInfo : GTWindow
{
    ...
    private void OnComposeClick(GameObject go)
    {
        ...
        if(item.ItemNum < sourceDB.Data1)
        {
    ...
    public void ShowViewByGuid(ulong guid)
    {
        ...
        sourceItemNum.text = GTTools.Format("拥有数量：{0}", item.ItemNum);
        ...
        targetItemNum.text = GTTools.Format("合成数量：{0}", item.ItemNum / sourceDB.Data1);
