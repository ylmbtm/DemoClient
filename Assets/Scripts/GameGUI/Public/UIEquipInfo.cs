...
public class UIEquipInfo : GTWindow
{
    ...
    protected override void OnAddHandler()
    {
        ...
        GTEventCenter.AddHandler<int, int>(GTEventID.TYPE_BAG_UNDRESS_EQUIP, OnRecvUnDressEquip);
    }

    protected override void OnDelHandler()
    {
        GTEventCenter.DelHandler<int, int>(GTEventID.TYPE_BAG_DRESS_EQUIP, OnRecvDressEquip);
        GTEventCenter.DelHandler<int, int>(GTEventID.TYPE_BAG_UNDRESS_EQUIP, OnRecvUnDressEquip);
    }
    ...
    {
         GTAudioManager.Instance.PlayEffectAudio(GTAudioKey.SOUND_UI_CLICK);
         GTItemHelper.ShowEquipWindowByGuid(mGuid);
    }

    private void OnDressClick(GameObject go)
    {
         GTAudioManager.Instance.PlayEffectAudio(GTAudioKey.SOUND_UI_CLICK);
         GTNetworkSend.Instance.TryDressEquip(mGuid, mBagGuid);
         Hide();
    }
    ...
    {
         GTAudioManager.Instance.PlayEffectAudio(GTAudioKey.SOUND_UI_CLICK);
         GTNetworkSend.Instance.TryUnDressEquip(mGuid);
         Hide();
    }
    ...
    private void OnRecvUnDressEquip(int arg1, int arg2)
    {
        Hide();
    }
    ...
    //装备的GUID
    public void ShowViewByGuid(ulong equipGuid, ulong bagGuid)
    {
        this.mGuid = equipGuid;
        this.mBagGuid = bagGuid;
        EquipItem equip = GTEquipData.GetItem(equipGuid);
        if (equip == null)
        {
            return;
        }
        
        ShowDress(equip.IsUsing > 0);

        int itemID = equip.EquipID;
        ShowBaseView(itemID);
        //equipFightValue.text   = GTTools.Format("战斗力 {0}", GTAttrHelper.GetFightValueForEquip(equip));
       // equipAdvanceLevel.text = MLEquip.Instance.GetEquipAdvanceNameByLevel(equip.AdvanceLevel);
       // List<int> attrValues   = GTAttrHelper.GetAttrValuesForEquip(equip);
        title3.text = GTTools.Format("装备星级 {0}", equip.StarLevel);
       // title2.text = GTTools.Format("进阶等级 {0}", equip.AdvanceLevel);
        //title1.text = GTTools.Format("强化等级 {0}", equip.StrengLevel);
       // GTItemHelper.ShowPropertyText(this.equipPropertys, attrValues, true);
