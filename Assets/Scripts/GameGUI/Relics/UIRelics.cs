...
public class UIRelics : GTWindow
{
    ...
    protected override void OnClose()
    {
        ...
        if (mModel != null)
        {
            GTResourceManager.Instance.DestroyObj(mModel);
            mModel = null;
        }
		
		if (mEffect != null)
        {
            GTResourceManager.Instance.DestroyObj(mEffect);
            mEffect = null;
        }
    }
    ...
    private void ShowRelicsObj()
    {
        ...
        mModel = GTResourceManager.Instance.Load<GameObject>(db.Model, true);
        if (mModel == null)
        {
            return;
        }
        mRender.AttachModel(mModel);
        mModel.SetActive(true);
        mModel.transform.localPosition = new Vector3(db.X, db.Y, db.Z);
        mModel.transform.localEulerAngles = new Vector3(0, 180, 0);
        mModel.transform.localScale = Vector3.one;
        mEffect = GTWorld.Instance.Effect.CreateEffectByID(db.ActiveEffectID);
        if (mEffect == null)
        {
            return;
        }

        mRender.AttachModel(mEffect);
        mEffect.transform.parent = mModel.transform;
        mEffect.transform.localPosition = Vector3.zero;
        mEffect.transform.localRotation = Quaternion.identity;
...
//             int itemID = db.ArtificeCostIDs[i];
...
//             tab.itemNum.text = GTTools.Format("当前拥有：[d96016]{0}[-]", GTData.Instance.GetItemCountById(itemID));
//             tab.tip.text = GTTools.Format("充能一次加 [d96916]{0}[-] 点经验", itemDB.Data1);
// 
//             if (relics == null)
//             {
//                 tab.chargeProgressValue.value = 0;
//                 tab.chargeProgressNum.text = GTTools.Format("{0}/{1}", 0, db.LevelExps[0]);
//             }
//             else
//             {
//                 int curExp = 0;
...
//                 }
// 
//                 int maxExp = db.LevelExps[relics.Level];
...
//             DProperty propertyDB = ReadCfgProperty.GetDataById(db.PropertyIDs[i]);
...
//             if (relics == null)
//             {
//                 num1 = ratio1 * db.PropertyNums[i];
//                 num2 = ratio2 * db.PropertyNums[i];
...
// 
//                 num1 = (db.PropertyNums[i] * ratio1);
//                 num2 = (db.PropertyNums[i] * ratio2);
// 
    ...
    private void ShowButtonsState()
    {
        ...
        if (relics != null)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                if (curExp < db.LevelExps[relics.Level])
                {
                    canUpgrade = false;
                }
            }
