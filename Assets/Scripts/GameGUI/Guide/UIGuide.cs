...
public class UIGuide : GTWindow
{
    ...
    public void ShowGuideBase(Guide data)
    {
        mask.SetActive(data.IsLocked);
        btnSkip.SetActive(data.IsCanSkip);
    ...
    public void ShowGuideTipEffect(GuideTip data)
    {
        ...
        else
        {
            ...
            this.tipTransform.localPosition = data.TipPosition;
            this.tipText.text = data.TipText;
        }
        switch(data.TipGirlPos)
        {
    ...
    public void ShowGuideLockEffect(GuideLock data)
    {
        Transform target = GTWorld.Instance.Guide.GetData(data.LockKey);
        if (target == null)
        {
            return;
        }
        ...
        GameObject btn = NGUITools.AddChild(pivot.gameObject, target.gameObject);
