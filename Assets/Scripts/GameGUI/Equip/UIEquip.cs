...
public class UIEquip : GTWindow
{
    ...
    private void OnAutoToAddMatClick(GameObject go)
    {
        ...
        EquipItem equip = GTEquipData.GetItem(mGuid);
...
//         ShowStrengthenView();
    ...
    {
         GTAudioManager.Instance.PlayEffectAudio(GTAudioKey.SOUND_UI_CLICK);
//         XEquip equip = GTData.Instance.GetEquipDataByPos(mPosType, mPos);
...
//         List<int> propertys = GTAttrHelper.GetAttrValuesForEquip(equip);
...
// 
//         int hasItemNum = GTData.Instance.GetItemCountById(nexStarDB.CostItemId);
...
// 
//         List<int> nowPropertys = GTAttrHelper.GetAttrValuesForEquip(equip);
//         XEquip nexEquip = new XEquip();
//         GTTools.Copy(nexEquip, equip);
//         nexEquip.AdvanceLevel++;
//         List<int> nexPropertys = GTAttrHelper.GetAttrValuesForEquip(nexEquip);
// 
...
//         int hasItemNum = GTData.Instance.GetItemCountById(db.CostItemId);
...
//         GTItemHelper.ShowEquipAdvanceText(equipAdvance.advanceText, itemID, equip.AdvanceLevel);
    ...
    public void ShowEquipWindowByGuid(ulong guid)
    {
        mGuid = guid;
        ShowView();
    }
