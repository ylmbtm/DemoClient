...
public class GTWindowManager : GTSingleton<GTWindowManager>
{
    ...
    void SetAllDepth()
    {
        ...
        m_MinDepths[EWindowType.MsgTip] = 2000;
        m_MinDepths[EWindowType.NewBie] = 4000;
        m_MinDepths[EWindowType.Loaded] = 5000;

        m_MinZOffsets[EWindowType.Bottom] = 0;
        m_MinZOffsets[EWindowType.Window] = -10000;
        m_MinZOffsets[EWindowType.MsgTip] = -50000;
        m_MinZOffsets[EWindowType.NewBie] = -60000;
        m_MinZOffsets[EWindowType.Loaded] = -65000;
    }
    ...
    void RegisterWindows()
    {
        ...
        RegisterWindow(EWindowID.UIPlotCutscene, new UIPlotCutscene());
    ...
    void RefreshDepth(GTWindow window)
    {
        ...
        {
            if (m_OpenWindows[i].Type == type && m_OpenWindows[i] != window)
            {
        ...
        Int32 stDepth   = m_MinDepths[type];
        float stZOffset = m_MinZOffsets[type];

        for (int i = 0; i < aList.Count; i++)
        {
            UIPanel item = aList[i];
            stDepth = item.depth > stDepth ? item.depth : stDepth;
            float z   = item.transform.localPosition.z;
            stZOffset = z < stZOffset ? z : stZOffset;
        }

        for (int i = 0; i < pList.Count; i++)
        {
            UIPanel item = pList[i];
            stDepth += 2;
            item.depth = stDepth;
            Vector3 pos = item.transform.localPosition;
            pos.z = stZOffset - 500 * (i + 1);
