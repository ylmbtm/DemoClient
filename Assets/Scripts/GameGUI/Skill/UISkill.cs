...
public class UISkill : GTWindow
{
    ...
    {
        for (int i = 0; i < MLSkill.Instance.TALENT_LEVELS.Length; i++)
        {
            GameObject item = null;
            if (i == 0)
            {
                item = mContentTemp2;
                item.transform.parent = mContentGrid2.transform;
            }
            else
            {
                item = NGUITools.AddChild(mContentGrid2.gameObject, mContentTemp2);
            }
            item.SetActive(true);
    ...
    private void ShowContentView2()
    {
        ...
        for (int i = 0; i < mTalents.Count; i++)
        {
            ...
            tab.talentLevel.text = MLSkill.Instance.TALENT_LEVELS[i].ToString();
            GTItemHelper.ShowImageBlack(tab.talentLevelBack, role.Level < MLSkill.Instance.TALENT_LEVELS[i]);
            activeArray[i] = 0;
        }
        while (em.MoveNext())
        {
            if (GTData.Main.Carrer != em.Current.Value.Carrer)
            {
                continue;
            }
